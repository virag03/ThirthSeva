...
namespace TirthSeva.API.Controllers
{
    ...
    public class BhaktnivasController : ControllerBase
    {
        ...
        public async Task<ActionResult<int>> GetAvailability(int id, [FromQuery] DateTime checkIn, [FromQuery] DateTime checkOut)
        {
            var capacity = await _bhaktnivasService.GetAvailableCapacityAsync(id, checkIn, checkOut);
            return Ok(capacity ?? 0);
        }

        [Authorize(Roles = "ServiceProvider,Admin")]
        [HttpGet("{id}/slots")]
        public async Task<ActionResult<List<BhaktnivasSlotDTO>>> GetSlots(int id, [FromQuery] DateTime? fromDate, [FromQuery] DateTime? toDate)
        {
            var slots = await _bhaktnivasService.GetSlotsAsync(id, fromDate, toDate);
            return Ok(slots);
        }

        [Authorize(Roles = "ServiceProvider")]
        [HttpPost("{id}/slots")]
        public async Task<ActionResult> ReleaseSlots(int id, [FromBody] ReleaseBhaktnivasSlotsRequest request)
        {
            var userIdClaim = User.FindFirst(ClaimTypes.NameIdentifier);
            if (userIdClaim == null) return Unauthorized();

            var userId = int.Parse(userIdClaim.Value);
            var result = await _bhaktnivasService.ReleaseSlotsAsync(id, request, userId);

            if (!result) return NotFound(new { message = "Bhaktnivas not found or unauthorized" });

            return Ok(new { message = "Slots released successfully" });
        }
    }
}
