...
export const bhaktnivasService = {
    ...
    getAvailability: async (id, checkIn, checkOut) => {
        const response = await api.get(`/bhaktnivas/${id}/availability?checkIn=${checkIn}&checkOut=${checkOut}`);
        return response.data;
    },

    getSlots: async (id, fromDate, toDate) => {
        let url = `/bhaktnivas/${id}/slots`;
        if (fromDate || toDate) {
            const params = new URLSearchParams();
    ...
    releaseSlots: async (id, data) => {
        const response = await api.post(`/bhaktnivas/${id}/slots`, data);
        return response.data;
    }
};
